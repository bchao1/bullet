...
class myInput:
    def __init__(self, word_color: str = colors.foreground["default"], password: bool = False, hidden: str = "*"):
...
class YesNo:
    def __init__(self, prompt, default="y", indent=0, word_color=colors.foreground["default"], prompt_prefix="[y/n] "):
        self.indent = indent
        if not prompt:
...
class Input:
    def __init__(self, prompt, default="", indent=0, word_color=colors.foreground["default"], strip=False, pattern=""):
        self.indent = indent
        if not prompt:
            raise ValueError("Prompt can not be empty!")
...
class VerticalPrompt:
    def __init__(self, components, spacing=1, separator="", separator_color=colors.foreground["default"]):
...
class ScrollBar:
    def __init__(
        ...
        self.max_width = len(max(self.choices, key=len)) + self.pad_right
        self.height = min(len(self.choices), height if height else len(self.choices))  # Size of the scrollbar window.
